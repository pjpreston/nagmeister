...
live APIs rather than recalled, because they differ in ways that matter:
    ...
    Anthropic   official SDK. tools take `input_schema`; a call comes back as
                ...
                search is Anthropic-hosted: declare web_search_20250305 and
...
def chat_anthropic(model_id, history, call, system, sources):
    ...
    tools = _anthropic_tools() + [
        ...
        #
        # The basic variant on purpose. web_search_20260209 adds dynamic
        # filtering, which runs code execution under the hood, and that does
        # not survive being mixed with our own tools: when a turn ends with
        # both one of our tool_use blocks and a still-pending code-execution
        # server_tool_use, the next request is rejected with
        #
        #   container_id is required when there are pending tool uses
        #   generated by code execution with tools
        #
        # and the id only appears on a later response, so there is nothing to
        # send at the point it is demanded. It failed about one question in
        # four. The basic variant creates no container, searches the same
        # sites, and is accepted on both Sonnet 5 and Opus 5 -- checked.
    ...
    for _ in range(MAX_STEPS):
        try:
            ...
            if "credit balance" in msg:
                raise ChatError("The Anthropic account is out of credit -- top it "
                                "up in Plans & Billing and ask again.")
            raise ChatError(f"Anthropic returned {e.status_code}: {msg}")
        ...
        if not calls:
            ...
            # a search-heavy turn can spend the budget mid-sentence; silently
            # handing back half an answer is worse than saying it is half
            if r.stop_reason == "max_tokens":
                if not text:
                    raise ChatError("The answer hit the length limit before any "
                                    "text was produced. Try a narrower question.")
